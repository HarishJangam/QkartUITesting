...
public class QKART_Tests {
    ...
    public void TestCase02() throws InterruptedException {
        ...
        assertFalse(status,"reRegistration failed");
    }
    ...
    public void TestCase03() throws InterruptedException {
        ...
        for (WebElement webElement : searchResults) {
            ...
            softAssert.assertTrue(status,"Test Results contains un-expected values");
        }
    ...
    public void TestCase04() throws InterruptedException {
        ...
        assertTrue(status,"not able to find running shoes");
        List<WebElement> searchResults = homePage.getSearchResults();
        ...
        for (WebElement webElement : searchResults) {
            ...
            assertTrue(status,"Validated presence of drop down failed");
            // WebDriverWait wait=new WebDriverWait(driver, 5);
            // wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//div[contains(@class,'css-uhb5lp')]")));
            status=result.openSizechart();
            // assertTrue(status,"unable to open size chart failed");
            status=result.validateSizeChartContents(expectedTableHeaders, expectedTableBody, driver);
            assertTrue(status,"size chart contenct verification failed");
    ...
    public void TestCase05(String YONEX,String Tan,String address) throws InterruptedException {
        ...
        // return status;
        assertTrue(status,"not went to thanks page");
    }
    ...
    public void TestCase06(String Xtend,String Yarine) throws InterruptedException {
        ...
        try {
            ...
            // return false;
            status=false;
            assertTrue(status,"Error while placing orde");
        }

        status = driver.getCurrentUrl().endsWith("/thanks");
        assertTrue(status,"Verify that cart can be edited failed");
        homePage.navigateToHome();
        homePage.PerformLogout();
    ...
    @Parameters({"Stylecon","a"})
    public void TestCase07(String Stylecon,int a) throws InterruptedException {
        Boolean status;
        ...
        homePage.changeProductQuantityinCart(Stylecon, a);
        ...
        logStatus("End TestCase",
                "Test Case 7: Verify that insufficient balance error is thrown when the wallet balance is not enough: ",
                status ? "PASS" : "FAIL");
            assertTrue(status,"Verify that insufficient balance error");
        // return status;
    }
    ...
    public void TestCase09() throws InterruptedException {
        ...
        //             status ? "PASS" : "FAIL");
        // }
            softAssert.assertTrue(status,"change on privacy policy link click failed");
        Set<String> handles = driver.getWindowHandles();
        driver.switchTo().window(handles.toArray(new String[handles.size()])[1]);
        ...
        //             status ? "PASS" : "FAIL");
        // }
            softAssert.assertTrue(status,"opened has Privacy Policy page heading failed");
        driver.switchTo().window(handles.toArray(new String[handles.size()])[0]);
        ...
        //             status ? "PASS" : "FAIL");
        // }
        softAssert.assertTrue(status,"opened has Terms Of Service page heading failed");
    ...
    public void TestCase10() throws InterruptedException {
        ...
        status=driver.findElement(By.xpath("//div[@class='card-block']")).isDisplayed();
        assertTrue(status,"contactUs not displayed");
    ...
    public void TestCase11() throws InterruptedException {
        ...
        assertTrue(status,"failed to search yonex");
        homePage.addProductToCart("YONEX Smash Badminton Racquet");
        ...
        softAssert.assertTrue(status,"Verify that 3 Advertisements are available failed");
        ...
        softAssert.assertTrue(status,"Verify that Advertisement 1 is clickable failed");
        driver.get(currentURL);
        Thread.sleep(3000);
        ...
        //         status ? "PASS" : "FAIL");
        // return status;
        softAssert.assertTrue(status,"Verify that Advertisement 2 is clickable failed");
